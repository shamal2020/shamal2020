//03/09/2020
//**This script is written for a USCG ACP Company Type that has Plan Review and NO Vessel permission.  If this changes, the script will have to be re-written as the location of the 
//xpaths changed depending on what permissions display.
//**To check to see if any new users have been added, the quickest way is to run the Customer List Report in ABS Admin, open the spreadsheet and filter on column Company Type = USCG ACP.
...
public class TC08CreateUserUSCGACP extends testEnvironment {
	...
	public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("\nTC1CreateUSCGACPUserOwner.main()--userBeforeCount = "+userBeforeCount);
	    ...
	    Thread.sleep(6000);
	    driver.findElement(By.xpath("//input[@name=\'userIdValue\']")).sendKeys("SanityAddUSCGACPUser"+ nxt); 
	  ...
	  //Capture User Count ** AFTER
	   ...
	   //Get after Count
        for (WebElement webElement : userAfterCountList) {
            System.out.println("TC1CreateUSCGACPUserOwner.main()--userAfterCount = " +webElement.getText());
            userAfterCount = Integer.parseInt(webElement.getText());
        } 
        
        if (userBeforeCount != 0 && userAfterCount != 0 && userBeforeCount < userAfterCount)
        {
        	System.out.println("TC1CreateUSCGACPUserOwner.main() -- SANITY TEST PASS");
	    } else {
	    	System.out.println("TC1CreateUSCGACPUserOwner.main() -- SANITY TEST FAIL");
	    }
